...
const pancakeAdress = "0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F";
...
// функция расчета
async function getTokensForUSDT(usdtAmount, contractAdress) {
  try {
...
async function getUSDTForTokens(tokenAmount, contractAdress, decimals) {
  try {
    // количество монет , которую отдаем и ее количество нулей (где узнать количество нулей?)
    const amountIn = decimals;
    // 1 адрес юсдт (отдаем) 0xdAC17F958D2ee523a2206206994597C13D831ec7
    // 2 адрес ефира (получаем) 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2
    const path = [contractAdress, "0xdAC17F958D2ee523a2206206994597C13D831ec7"];
    // расчитываем сделку
    const amounts = await contract.getAmountsOut(amountIn, path);
    ...
    if (examinationPair === "0x0000000000000000000000000000000000000000") {
      ...
      try {
        for (const amount of coundAmount) {
          try {
            console.log(coin.decimals, "for", coin.name);
            let tokenAmount = await getTokensForUSDT(amount, coin.contract);
            let usdtAmount = await getUSDTForTokens(
              tokenAmount,
              coin.contract,
              coin.decimals
            );
